...
/* metadata (for subscription and record locking) */
var metadata = {};
...
/* public methods */
module.exports.select = function(obj,pattern) {
	selection = [];
	var keys = splitPath(pattern);
	traverse(obj,keys,"");
	return selection;
}
...
module.exports.update = function(obj,data) {
	...
	for(var d=0;d<data.length;d++) {
		var record = data[d];
		var keys = splitPath(record.path);
		traverse(obj,keys,"");
		//log("updating path: " + record.path + "/" + record.key + " found: " + selection[d].path + "/" + selection[d].key);
		selection[d].value[record.key] = record.value;
	}
