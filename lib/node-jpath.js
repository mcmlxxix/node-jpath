...
var conditions = {
	...
	">": 	(l, r) => l > r,
	">=": 	(l, r) => l >= r,
	"~=": 	(l, r) => (l + EMPTY).toLowerCase() === (r + EMPTY).toLowerCase(),
	"^=": 	(l, r) => !((l + EMPTY).indexOf(r)),
	"$=": 	(l, r) => (r + EMPTY) === (l + EMPTY).slice(-(r + EMPTY).length),
	"*=": 	(l, r) => (l + EMPTY).toLowerCase().indexOf((r + EMPTY).toLowerCase()) !== -1
};
...
function matchCondition(condition, obj) {
	...
	var evalStr = condition.replace(rxPairs, (match, pl, left, operator, right, pr) =>
		[pl, testPairs.call(obj, left, right, operator/*, cfn*/), pr].join(EMPTY));
	return eval(evalStr);
}
...
module.exports.settings = new (function(){
	this.__defineGetter__('delimiter',() => delimiter);
	this.__defineSetter__('delimiter',(value) => {if(!(rxIllegal.test(value))) delimiter = value});
	this.__defineGetter__('create',() => create);
	this.__defineSetter__('create',(value) => {if(typeof value == "boolean") create = value});
})();
